...
pub enum Error {
    ...
    #[error("swe_edge_http_cassette: config parse failed — {0}")]
    ParseFailed(String),

    /// Middleware behavior not yet implemented (scaffold phase).
    #[error("swe_edge_http_cassette: not implemented — {0}")]
...
mod tests {
    ...
    fn test_not_implemented_display_includes_crate_name() {
        ...
        assert!(err.to_string().contains("swe_edge_http_cassette"));
    }
    ...
    fn test_parse_failed_display_names_crate_and_reason() {
        ...
        let s = err.to_string();
        assert!(s.contains("swe_edge_http_cassette"));
