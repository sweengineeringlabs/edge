...
use crate::api::database::{
    DatabaseGateway, DatabaseRead, DatabaseWrite, QueryParams, Record, WriteResult,
};
...
impl DatabaseWrite for MemoryDatabase {
    fn insert(&self, table: &str, record: Record) -> BoxFuture<'_, EgressResult<WriteResult>> {
        ...
        Box::pin(async move {
            ...
            if t.contains_key(&id) {
                return Err(EgressError::AlreadyExists(format!("id={id}")));
            }
    ...
    fn update(&self, table: &str, id: &str, record: Record) -> BoxFuture<'_, EgressResult<WriteResult>> {
        ...
        Box::pin(async move {
            ...
            let t = guard.entry(table).or_default();
            if !t.contains_key(&id) {
                return Err(EgressError::NotFound(format!("id={id}")));
            }
            t.insert(id, record);
...
mod tests {
    ...
    async fn test_update_nonexistent_record_returns_not_found_error() {
        ...
        assert!(matches!(r, Err(EgressError::NotFound(_))));
    }
