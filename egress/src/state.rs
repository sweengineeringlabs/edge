...
impl<S: ?Sized> CachedService<S> {
    ...
    pub async fn get_or_init<F, Fut>(&self, init: F) -> EgressResult<Arc<S>>
    where F: FnOnce() -> Fut, Fut: Future<Output = EgressResult<Arc<S>>>,
    {
...
impl<S: ?Sized, C: PartialEq + Clone> ConfiguredCache<S, C> {
    ...
    pub async fn get_or_init_with_config<F, Fut>(&self, config: &C, init: F) -> EgressResult<Arc<S>>
    where F: FnOnce(C) -> Fut, Fut: Future<Output = EgressResult<Arc<S>>>,
    {
