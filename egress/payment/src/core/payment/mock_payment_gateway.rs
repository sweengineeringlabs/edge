...
use crate::api::port::{PaymentError, PaymentGateway, PaymentInbound, PaymentOutbound, PaymentPortResult};
use crate::api::value_object::{
    PaymentMethod, PaymentResult, PaymentStatus, Refund, RefundResult, RefundStatus,
};
...
impl PaymentInbound for MockPaymentGateway {
    ...
    fn list_payment_methods(&self, _customer_id: &str) -> BoxFuture<'_, PaymentPortResult<Vec<PaymentMethod>>> {
...
impl PaymentOutbound for MockPaymentGateway {
    ...
    fn create_customer(&self, customer: Customer) -> BoxFuture<'_, PaymentPortResult<Customer>> {
        Box::pin(async move {
            if self.customers.read().contains_key(&customer.id) {
                return Err(PaymentError::AlreadyExists(format!("customer {}", customer.id)));
            }
    ...
    fn attach_payment_method(&self, _customer_id: &str, method: PaymentMethod) -> BoxFuture<'_, PaymentPortResult<PaymentMethod>> {
...
mod tests {
    use super::*;
    use crate::api::value_object::{Money, PaymentStatus, Refund, RefundReason};
    ...
    async fn test_create_duplicate_customer_returns_already_exists_error() {
        ...
        assert!(matches!(r, Err(PaymentError::AlreadyExists(_))));
    }

    #[tokio::test]
    async fn test_refund_returns_succeeded_status() {
        let g = gw();
        let r = g.refund(Refund::full("pay-1", RefundReason::RequestedByCustomer)).await.unwrap();
